...
export function RatingGameCard({ game, matchId, currentUserId, isAdmin }: Props) {
  const isExpired = game.rating_opened_at
    ? Date.now() - new Date(game.rating_opened_at).getTime() > 48 * 60 * 60 * 1000
    : false;

  const isOpen = game.rating_open && !isExpired;

  const otherPlayers = game.Match.DraftPick.filter((dp) => dp.user_id !== currentUserId);

  const initField: Record<string, number> = {};
  const initGk: Record<string, number> = {};
  for (const r of game.GameRating) {
    ...
    <section className="rounded-2xl border border-white/[0.08] overflow-hidden">
      <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-white/[0.06]">
        <div className="flex items-center gap-2">
          <p className="text-sm font-semibold">Partita {game.game_number}</p>
          {isOpen ? (
