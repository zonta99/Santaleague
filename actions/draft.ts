...
export const executeDraft = async (matchId: number) => {
  ...
  // Fetch level for each participant (outside transaction — read-only, no side effects)
  ...
  const teams = await db.$transaction(async (tx) => {
    ...
    const existingPicks = await tx.draftPick.findMany({ where: { match_id: matchId }, select: { team_id: true } });
    if (existingPicks.length > 0) {
      const existingTeamIds = Array.from(new Set(existingPicks.map((p) => p.team_id)));
      await tx.draftPick.deleteMany({ where: { match_id: matchId } });
      await tx.game.updateMany({ where: { match_id: matchId }, data: { team1_id: null, team2_id: null } });
      await tx.teamMember.deleteMany({ where: { team_id: { in: existingTeamIds } } });
      await tx.team.deleteMany({ where: { id: { in: existingTeamIds } } });
    }

    // Create teams
    const newTeams = await Promise.all(
      teamNames.map((name) => tx.team.create({ data: { name: `${matchDate} - ${name}`, team_type: null } }))
    );
...
export const lockDraft = async (matchId: number) => {
  ...
  await db.$transaction(async (tx) => {
    ...
    for (const [teamId, teamPicks] of Array.from(teamPicksMap.entries())) {
      const existingCaptain = await tx.teamMember.findFirst({ where: { team_id: teamId, is_captain: true } });
...
export const movePlayer = async (matchId: number, userId: string, toTeamId: number) => {
  ...
  await db.$transaction(async (tx) => {
    await tx.draftPick.update({ where: { id: pick.id }, data: { team_id: toTeamId } });

    // Re-index pick_order for both affected teams
    for (const teamId of [fromTeamId, toTeamId]) {
      const teamPicks = await tx.draftPick.findMany({
        where: { match_id: matchId, team_id: teamId },
        orderBy: { pick_order: "asc" },
      });
      for (let i = 0; i < teamPicks.length; i++) {
        await tx.draftPick.update({ where: { id: teamPicks[i].id }, data: { pick_order: i + 1 } });
      }
    }
  });
...
export const setCaptain = async (matchId: number, teamId: number, userId: string) => {
  ...
  await db.$transaction(async (tx) => {
    await tx.teamMember.updateMany({ where: { team_id: teamId }, data: { is_captain: false } });
    await tx.teamMember.upsert({
...
export const resetDraft = async (matchId: number) => {
  ...
  await db.$transaction(async (tx) => {
    ...
    if (teamIds.length > 0) {
      await tx.teamMember.deleteMany({ where: { team_id: { in: teamIds } } });
